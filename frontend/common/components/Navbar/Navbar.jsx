...
const Navbar = () => {
  ...
  return (
    <>
      ...
      {isScrolled && <div className={`fixed top-0 z-40 w-full h-auto flex items-center duration-500 border-b-[1px] border-[#cbcbcb79] ${isActive ? 'active' : ''} bg-white`}>

        <div className={`w-[100vw] h-full flex items-center justify-between text-left lg:px-20 px-5 transition-all py-6`}>
          <div className='flex flex-row items-center justify-center gap-2 text-xl'>
            <IoHome className={`mb-1 text-black`} />
            <a href="/" className={`text-xl font-bold text-black`}> Bashera</a>
          </div>

          <nav className={`fixed lg:flex hidden flex-row justify-center items-center w-full h-auto gap-8 text-base font-light text-black bg-white`}>
            <Link href="/">Home</Link>
            <Link href="/Services">Services</Link>
            <Link href="/About">About</Link>
            <Link href="/Contact">Contact</Link>
